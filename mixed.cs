...
namespace TodoApp
{
  ...
  
    {
      ...
      string GorevBaslik { get; set; }
        string GorevAciklama { get; set; }
        int    GorevDurum   { get; set; }   // 1=Todo, 2=Inprogress, 3=Done
        int    GorevOncelik { get; set; }   // 1=Low, 2=Medium, 3=High
        string GorevBaslangic { get; set; }
        string GorevBitis      { get; set; }
    ...
    //ortak küçük yardımcıları sağlar, metotları abstract bırakarak alt sınıfa override zorunluluğu getirir.
    {
        public string GorevBaslik { get; set; } = "";
        public string GorevAciklama { get; set; } = "";
        public int    GorevDurum { get; set; }
        public int    GorevOncelik { get; set; }
        public string GorevBaslangic { get; set; } = "";
        public string GorevBitis { get; set; } = "";

        // (İsteğe bağlı) küçük doğrulama yardımcıları:
        protected bool IsValidDurum(string? s)   => s == "1" || s == "2" || s == "3";
        protected bool IsValidOncelik(string? s)=> s == "1" || s == "2" || s == "3";

        public abstract void TaskOlustur();
        public abstract void Guncelle();
    }


    public class Tasks : TaskBase
    //TaskBase'ten kalıtım alır ve metotları override eder.
    {
        public override void TaskOlustur()
        {
        ...
        public override void Guncelle()
        {
            ...
            if (IsValidOncelik(oncelikSecimi))
    ...
    class Program
     //Program (akış aynı, sadece List<ITask> kullanıyoruz)
    {
        static List<ITask> _gorevListesi = new List<ITask>();
        static IMenu _menu = new Menu();
                ...
                switch (secim)
                {
                    ...
                    case "6":
                        ...
                        else
                        {
                            ...
                            List<ITask> sonuc = new List<ITask>();
